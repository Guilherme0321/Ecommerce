...
import { validUsername } from "../shared/service/usernameIsUnique";
import { authUser } from "../shared/components/utils/authUserInputs";
import { validUserEmail } from "../shared/service/userEmailisUnique";
export const Register = () => {
    ...
    const setErros = async (statusUser: StatusUser | undefined): Promise<StatusUser | undefined> => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const statusUser: StatusUser | undefined = await setErros(authUser(user));

        if(statusUser !== undefined && statusUser?.username && statusUser?.name && statusUser?.email && statusUser.address?.city && statusUser.address?.number && statusUser.address?.street){
    ...
    return (    
        <div className="position-absolute top-50 start-50 translate-middle bg-white bg-opacity-75 rounded-4" style={{width:'25%'}}>
